-- Filespec: $Id: install_funcs.sql.in 21 2007-06-11 16:11:28Z rmattes $
...
CREATE OR REPLACE FUNCTION facts_uri_in(opaque)
  RETURNS facts_uri
  AS '/usr/lib/postgresql/8.3/lib/facts.so', 'facts_uri_in'
  LANGUAGE 'C';

CREATE OR REPLACE FUNCTION facts_uri_out(opaque)
  RETURNS cstring
  AS '/usr/lib/postgresql/8.3/lib/facts.so', 'facts_uri_out'
  LANGUAGE 'C';
...
CREATE OR REPLACE FUNCTION facts_uri_eq(facts_uri, facts_uri)
  RETURNS bool
  AS '/usr/lib/postgresql/8.3/lib/facts.so', 'facts_uri_eq'
  LANGUAGE 'C';

CREATE OR REPLACE FUNCTION facts_uri_is_parent_or_self(facts_uri, facts_uri)
  RETURNS bool
  AS '/usr/lib/postgresql/8.3/lib/facts.so', 'is_parent_or_self'
  LANGUAGE 'C';
...
CREATE OR REPLACE FUNCTION assert(text, text, text, text) 
  RETURNS bool
  AS '/usr/lib/postgresql/8.3/lib/facts.so', 'facts_assert'
  LANGUAGE 'C' STABLE;

CREATE OR REPLACE FUNCTION forget(text, text, text) 
  RETURNS bool
  AS '/usr/lib/postgresql/8.3/lib/facts.so', 'facts_forget'
  LANGUAGE 'C' STABLE;

CREATE OR REPLACE FUNCTION delete_resource(text) 
  RETURNS bool
  AS '/usr/lib/postgresql/8.3/lib/facts.so', 'facts_delete_resource'
  LANGUAGE 'C';
 
CREATE OR REPLACE FUNCTION move(text, text) 
  RETURNS bool
  AS '/usr/lib/postgresql/8.3/lib/facts.so', 'facts_move_all'
  LANGUAGE 'C' STABLE;
